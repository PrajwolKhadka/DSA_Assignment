...
public class MinimumRewards {
    ...
    public static int minRewards(int[] ratings) {
        ...
        Arrays.fill(rewards, 1); // Step 1: Assign 1 reward to every employee
        
        // Step 2: Traverse from left to right, increasing rewards for increasing ratings
        for (int i = 1; i < n; i++) {
        ...
        // Step 3: Traverse from right to left, ensuring decreasing ratings are considered
        for (int i = n - 2; i >= 0; i--) {
            if (ratings[i] > ratings[i + 1]) {
        ...
        // Step 4: Sum up the total rewards needed
        int totalRewards = 0;
